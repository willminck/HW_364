...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/wminckHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('search_gifs',db.Column('term_id',db.Integer, db.ForeignKey('term.id')),db.Column('gif_id',db.Integer, db.ForeignKey('gifs.id')))
...
user_collection = db.Table('user_collection',db.Column('gif_id',db.Integer, db.ForeignKey('gifs.id')),db.Column('collection_id',db.Integer, db.ForeignKey('collection.id')))
...
def get_or_create_gif(title, url):
    ...
    gif = db.session.query(Gif).filter_by(title=title, embedURL=url).first()
    if gif:
        return gif
    else:
...
def get_or_create_search_term(term):
    ...
    if search_term:
        ...
        for gif in gif_data:
            new = get_or_create_gif(title=gif['title'], url=gif['embed_url'])
            search_term.gifs.append(new)
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    collection = db.session.query(PersonalGifCollection).filter_by(name=name, user=current_user).first()
    if collection:
        return collection
    else:
        collection = PersonalGifCollection(name=name, user=current_user)
        for gif in gif_list:
            collections.gifs.append(collection)
...
def create_collection():
    ...
    if form.validate_on_submit():
        all_picks = []
        for pick in form.gif_picks.choices:
            new = get_gif_by_id(id=pick[0])
            all_picks.append(new)
        get_or_create_collection(name=form.name.data, current_user=current_user, gif_list=all_picks)
        return redirect(url_for('collections', form=form))
    else:
